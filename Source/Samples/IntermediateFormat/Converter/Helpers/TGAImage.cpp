...
IFXRESULT TGAImage::Write( const IFXCHAR* pFileName )
{
	TgaHeader header;
    IFXRESULT ret = IFX_OK;
	FILE* outFile = 0;
	...
	if( IFXSUCCESS( ret ) && NULL == m_BGRPixels )
	{
		// first attemp to write TGA image
		m_BGRPixels = new U8[ m_Width * m_Height * m_Channels ];
		if( NULL != m_BGRPixels && NULL != m_RGBPixels )
		{
			// R and B channels reordering
			U32 i;
			for( i = 0; i < m_Width * m_Height * m_Channels; i += m_Channels )
			{    
				m_BGRPixels[i] = m_RGBPixels[i+2]; // R->B
				m_BGRPixels[i+1] = m_RGBPixels[i+1]; // G->G
				m_BGRPixels[i+2] = m_RGBPixels[i]; // B->R
				if( 4 == m_Channels )
					m_BGRPixels[i+3] = m_RGBPixels[i+3]; // A->A
			}
		}
		else
			ret = IFX_E_OUT_OF_MEMORY;
	...
	{
		size_t count = 
			fwrite( m_BGRPixels, m_Width * m_Height * m_Channels, 1, outFile );
