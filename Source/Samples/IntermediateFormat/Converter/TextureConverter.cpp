...
IFXRESULT TextureConverter::Convert()
{
	...
	if( IFXSUCCESS( result ) )
	{
		...
		if( m_pTexture->IsExternal() == FALSE )
		{
			...
			{
				wprintf( L" -- Error - could not load %ls\n", file );
			...
			if( IFXSUCCESS( result ) )
			{
				...
				{
					printf("Error: Format defined do not match actual format of TGA\n");
				...
				if( IFXSUCCESS( result ) && 0 != m_limit )
				{
					...
					if( ( sourceImageInfo.m_width > m_limit ) || 
						...
						printf( "-- Warning: texture size was changed because it exceeds"
