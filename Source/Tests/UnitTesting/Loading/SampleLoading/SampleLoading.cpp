...
IFXRESULT SampleLoading::Run()
{
	...
	if(IFXSUCCESS(result))
	{
		...
		if(IFXSUCCESS(result))
		{
			NonAtomic( pCoreServices, pSceneGraph );
			Atomic( pCoreServices, pSceneGraph );
		}
	}
...
void SampleLoading::NonAtomic( IFXCoreServices* pCoreServices, IFXSceneGraph* pSceneGraph )
{
	...
	IFXString src, src_out;
	{
		...
		src+="Tests/UnitTesting/Loading/SampleLoading/box.u3d";
...

	{
		...
		src_out+="Tests/UnitTesting/Loading/SampleLoading/box_non_out.u3d";
...
void SampleLoading::Atomic( IFXCoreServices* pCoreServices, IFXSceneGraph* pSceneGraph )
{
	IFXString src;
	src = getenv("U3D_SOURCE");
	src+="Tests/UnitTesting/Loading/SampleLoading/box.u3d";
	...
	}
	{
		...
		src = getenv("U3D_SOURCE");
		src+="Tests/UnitTesting/Loading/SampleLoading/box_out.u3d";
