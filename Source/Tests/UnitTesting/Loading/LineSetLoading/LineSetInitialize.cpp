...
#define EPS 1.E-5
  ...
  IFXAuthorLineSetDesc desc;
    ...
    for (i=0; i < pLineSet->GetLineSetDesc()->m_numTexCoords; i++) {
      ...
      pLineSet->SetTexCoord( i, &(IFXVector4( tcoord+i, tcoord+1+i, tcoord+2+i, tcoord+3+i )) );
    }
...
#define EPS 1.E-5
  ...
  IFXAuthorLineSetDesc desc;
    ...
    IFXU32Line Line;
    Line.Set(0,1);  pLineSet->SetPositionLine(0, &Line);
    Line.Set(2,3);  pLineSet->SetPositionLine(1, &Line);
    Line.Set(4,5);  pLineSet->SetPositionLine(2, &Line);
    Line.Set(6,7);  pLineSet->SetPositionLine(3, &Line);
    Line.Set(8,9);  pLineSet->SetPositionLine(4, &Line);    
	Line.Set(10,11);  pLineSet->SetPositionLine(5, &Line);
	Line.Set(12,13);  pLineSet->SetPositionLine(6, &Line);
	Line.Set(14,15);  pLineSet->SetPositionLine(7, &Line);
	Line.Set(16,17);  pLineSet->SetPositionLine(8, &Line);
...
//#define EPS 1.E-5
  ...
  IFXAuthorLineSetDesc desc;
    desc.m_numLines			= 5;
    desc.m_numPositions		= 4;
    desc.m_numNormals			= 4;
    desc.m_numDiffuseColors	= 1;
    desc.m_numSpecularColors	= 0;
    desc.m_numTexCoords		= 0;
    desc.m_numMaterials		= 1;
    ...
    IFXVector3* pVector = &buff;
	i = 0;
    pVector->X() = 0.0f;  pVector->Y() = 10.0f;  pVector->Z() = 0.0f;  pLineSet->SetPosition(i++,pVector);
    pVector->X() = 10.0f; pVector->Y() = 10.0f;  pVector->Z() = 0.0f;  pLineSet->SetPosition(i++,pVector);
    pVector->X() = 0.0f;  pVector->Y() = 0.0f;  pVector->Z() = 0.0f;  pLineSet->SetPosition(i++,pVector);
    pVector->X() = 10.0f; pVector->Y() = 0.0f;  pVector->Z() = 0.0f;  pLineSet->SetPosition(i++,pVector);

	// 1.A - LINES
	IFXU32Line Line; i = 0;
	Line.Set(0,1);  pLineSet->SetPositionLine(i++, &Line);
	Line.Set(0,2);  pLineSet->SetPositionLine(i++, &Line);
	Line.Set(1,3);  pLineSet->SetPositionLine(i++, &Line);
	Line.Set(2,3);  pLineSet->SetPositionLine(i++, &Line);
	Line.Set(0,3);  pLineSet->SetPositionLine(i++, &Line);

	// 2 - NORMALS
	i = 0;
    pVector->X() = 0.0f;  pVector->Y() = 0.5f;  pVector->Z() = 0.5f;  pLineSet->SetNormal(i++,pVector);
    pVector->X() = 0.0f;  pVector->Y() = 0.2f;  pVector->Z() = 0.8f;  pLineSet->SetNormal(i++,pVector);
    pVector->X() = 0.0f;  pVector->Y() = 0.5f;  pVector->Z() = 0.5f;  pLineSet->SetNormal(i++,pVector);
    pVector->X() = 0.0f;  pVector->Y() = 0.2f;  pVector->Z() = 0.8f;  pLineSet->SetNormal(i++,pVector);
