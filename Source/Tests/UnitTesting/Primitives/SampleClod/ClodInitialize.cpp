...
#define EPS 1.E-5
  ...
  if (!pClodMesh) return IFX_E_INVALID_POINTER;
	 ...
	 desc.NumBaseVertices	= _NUMBASEVERTICES;
	 desc.NumPositions		= _NUMPOS;
	 desc.NumNormals		= _NUMNORM;
	 desc.NumFaces			= _NUMFACES;
	 desc.NumSpecularColors = _NUMSPECCOLORS;
	 desc.NumTexCoords		= _NUMTEXCOORD;
	 desc.NumMaterials		= _NUMMATERIAL;
	 desc.NumDiffuseColors	= _NUMDIFFCOLORS;
	 pClodMesh->Allocate(&desc);
	 pClodMesh->SetMeshDesc(&desc);
	// pClodMesh->SetMaxResolution(3);
	 //pClodMesh->SetMinResolution(3);
     U32 i;
     int num = 0;
...
//
     // 1 - POSITIONS
     IFXVector3* pVector = new IFXVector3;
     pVector->X() = 0.0f;  pVector->Y() = 0.0f;   pVector->Z() = 0.0f;  pClodMesh->SetPosition(0,pVector);
     pVector->X() = 10.0f; pVector->Y() = 0.0f;   pVector->Z() = 0.0f;  pClodMesh->SetPosition(1,pVector);
	 pVector->X() = 0.0f;  pVector->Y() = 10.0f;  pVector->Z() = 0.0f;  pClodMesh->SetPosition(2,pVector);



    // 1.A - TRIANGLES(FACES)
	IFXAuthorFace Face;
	Face.Set(1,0,2); pClodMesh->SetPositionFace(0,&Face);


    // 2 - NORMALS
    pVector->X() = 0.0f;  pVector->Y() = 0.0f;  pVector->Z() = 1.0f;    pClodMesh->SetNormal(0,pVector);
    pVector->X() = 0.5f;  pVector->Y() = 0.0f;  pVector->Z() = 0.5f;    pClodMesh->SetNormal(1,pVector);
    pVector->X() = 0.0f;  pVector->Y() = 0.5f;  pVector->Z() = 0.5f;    pClodMesh->SetNormal(2,pVector);
 ...
 //  NORMAL and DIFFUSE COLOR at FACE
	...
	if( pClodMesh->GetMeshDesc()->NumSpecularColors > 0 )
	{
	...
	for (i=0; i < pClodMesh->GetMeshDesc()->NumTexCoords; i++) {
	  ...
	  pClodMesh->SetTexCoord( i, new IFXVector4( tcoord+i, tcoord+1+i, tcoord+2+i, tcoord+3+i ) );
    }
 ...
 // 6 - MATERIALS
	...
	for (i=0; i < pClodMesh->GetMeshDesc()->NumMaterials; i++) {
        ...
        pClodMesh->SetMaterial( i, &material );
    }
...
// 7 - FACE MATERIALS
	for (i=0; i < pClodMesh->GetMeshDesc()->NumFaces; i++)
        {
                pClodMesh->SetFaceMaterial(i,0);
    }


 // return ir;
return IFX_OK;
}// Init_Simplest_AuthorMesh()
...
//
	...
	// 3 - DIFFUSE
	IFXVector4 diff_color( 1.0f, 0.0f, 0.0f, 1.0f );
	IFXVector4 diff_color1( 0.0f, 1.0f, 0.0f, 1.0f );
 ...
 //  NORMAL and DIFFUSE COLOR at FACE
	...
	material_0.m_uOriginalMaterialID  = 1;
...
// materials width ID  1, 2, 3,  are rendered very strange, probably not correct
      ...
      if( i==1 )
        pClodMesh->SetMaterial( i, &material_1 );

          if( i==2 )
        pClodMesh->SetMaterial( i, &material_2 );

          if( i==3 )
        pClodMesh->SetMaterial( i, &material_3 );
    ...
    for (i=0; i < pClodMesh->GetMeshDesc()->NumFaces; i++) {
                ...
                pClodMesh->SetFaceMaterial(i,i%2);
	}
