...
{
	return IFXOSSocketConnect( m_socket, pServer, port );
}

I32 IFXSocketStream::Receive( I8* pBuffer, I32 nBytes )
{
	return IFXOSSocketReceive( m_socket, pBuffer, nBytes, 0 );
}

I32 IFXSocketStream::Send( const I8* pBuffer, I32 nBytes )
{
	return IFXOSSocketSend( m_socket, pBuffer, nBytes, 0 );
}

IFXRESULT IFXSocketStream::SendBytes( const I8* pBuffer )
{
	IFXRESULT result = IFX_OK;
	I32 len = strlen( pBuffer );

	I32 bytesSent = IFXOSSocketSend( m_socket, pBuffer, len, 0 );
								...
								U32* pNumberOfBytesRead )
{
	...
	if( IFXSUCCESS( result ) )
	{
		...
		{
			pBufferPtr += nread;
			nread = IFXOSSocketReceive( m_socket, pBufferPtr, nleft, 0);
...
{
	return IFXOSSocketSetOptions( m_socket, option, pBuffer, bufferLength );
}


IFXRESULT IFXSocketStream::GetOptions( U32 option, I8* pBuffer, I32* pBufferLength )
{
	return IFXOSSocketGetOptions( m_socket, option, pBuffer, pBufferLength );
}
