...
const I32 MAX_PORT_DIGIT_COUNT = 5;
const I32 MAX_SCHEMA_LEN = 7;
...
void IFXInternetSessionX::OpenX( IFXCHAR* pUrl )
{
...
void IFXInternetSessionX::OpenX( I8* pUrl )
{
	...
	try
	{
		...
		if( parseData.hostLen > 0 )
		{
			...
			strncpy( pHost, parseData.pHost, parseData.hostLen );
				...
				struct UrlParseData& rParseData )
{
	...
	// parse schema
	pPtr = strstr( pUrl, SCHEMA_TERMINATOR ); 
	...
	if( IFXSUCCESS( result ) )
	{
		...
		else
		{
			...
			I8* pPassword = strchr( pPtr, USER_DELIMITER );
	...
	if( IFXSUCCESS( result ) )
	{
		...
		pPtr = strchr( pHost, PORT_DELIMITER );

		// look for host name terminator
		I8* pFilePath = strchr( pHost, HOST_TERMINATOR );
		...
		else
		{
			if( NULL != pPtr )
			{
				...
				I32 digitCount = pFilePath - pPtr;

				if( digitCount <= MAX_PORT_DIGIT_COUNT )
				{
					strncpy( port, pPtr, digitCount );
					port[digitCount] = 0;

					rParseData.port = atoi( port );
				}
				else
				{
