...
IFXRESULT CIFXDeviceLightOGL::SetHWType(IFXenum eType)
{
	...
	case IFX_DIRECTIONAL:
		...
		m_pOGL->glLightf(m_uIndex, GL_SPOT_CUTOFF, 180);
		break;
	case IFX_SPOT:
...
IFXRESULT CIFXDeviceLightOGL::SetHWAmbient(const IFXVector4& vColor)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glLightfv(m_uIndex, GL_AMBIENT, vScaledColor.RawConst());
...
IFXRESULT CIFXDeviceLightOGL::SetHWDiffuse(const IFXVector4& vColor)
{
	...
	vScaledColor.Scale3(m_fIntensity);
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glLightfv(m_uIndex, GL_DIFFUSE, vScaledColor.RawConst());
...
IFXRESULT CIFXDeviceLightOGL::SetHWSpecular(const IFXVector4& vColor)
{
	...
	vScaledColor.Scale3(m_fIntensity);
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glLightfv(m_uIndex, GL_SPECULAR, vScaledColor.RawConst());
...
IFXRESULT CIFXDeviceLightOGL::SetHWAttenuation(const IFXVector3& vAttenuation)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glLightf(m_uIndex, GL_CONSTANT_ATTENUATION, m_vAttenuation[0]);
	m_pOGL->glLightf(m_uIndex, GL_LINEAR_ATTENUATION, m_vAttenuation[1]);
	m_pOGL->glLightf(m_uIndex, GL_QUADRATIC_ATTENUATION, m_vAttenuation[2]);
...
IFXRESULT CIFXDeviceLightOGL::SetHWSpotDecay(BOOL bEnabled)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
		m_pOGL->glLightf(m_uIndex, GL_SPOT_EXPONENT, fSpotExponent); 
	}
	else
	{
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
		m_pOGL->glLightf(m_uIndex, GL_SPOT_EXPONENT, 0.0f);
	}
...
IFXRESULT CIFXDeviceLightOGL::SetHWSpotOuterAngle(F32 fAngle)
{
	...
	{
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
		m_pOGL->glLightf(m_uIndex, GL_SPOT_CUTOFF, fAngle);
	}
...
IFXRESULT CIFXDeviceLightOGL::SetHWEnabled(BOOL bEnabled)
{
	...
	{
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
		m_pOGL->glEnable(m_uIndex);
	}
	else
	{
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
		m_pOGL->glDisable(m_uIndex);
	}
...
IFXRESULT CIFXDeviceLightOGL::SetHWWorldMatrix(const IFXMatrix4x4& mWorldMatrix)
{
	...
	{
	case IFX_SPOT:
		m_pOGL->glLightfv(m_uIndex, GL_SPOT_DIRECTION, ms_vNegativeZ.RawConst());
	case IFX_POINT:
		m_vPosition.Set(0,0,0,1);
		m_pOGL->glLightfv(m_uIndex, GL_POSITION, m_vPosition.RawConst());
		break;
	case IFX_DIRECTIONAL:
