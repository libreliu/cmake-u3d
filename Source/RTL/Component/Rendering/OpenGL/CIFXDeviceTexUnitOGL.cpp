...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWEnabled(BOOL bEnabled)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
		if(m_bCubeMap)
		{
			m_pOGL->glEnable(GL_TEXTURE_CUBE_MAP_ARB);
		}
		else
		{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
		if(m_bCubeMap)
		{
			m_pOGL->glDisable(GL_TEXTURE_CUBE_MAP_ARB);
		}
		else
		{
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWAlphaFunc(IFXenum eFunc)
{
	...
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_ALPHA_EXT, eGLFunc);
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWAlphaInput0(IFXenum eInput)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_ALPHA_EXT, eSrc);
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_ALPHA_EXT, eOperand);
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWAlphaInput1(IFXenum eInput)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_ALPHA_EXT, eSrc);
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND1_ALPHA_EXT, eOperand);
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWAlphaInterpolatorSource(IFXenum eInput)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE2_ALPHA_EXT, eSrc);
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_ALPHA_EXT, eOperand);
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWRGBBlendFunc(IFXenum eFunc)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glTexEnvf(GL_TEXTURE_ENV, GL_RGB_SCALE_EXT, fScale);
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_RGB_EXT, eGLFunc);
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWRGBInput0(IFXenum eInput)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_RGB_EXT, eSrc);
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_RGB_EXT, eOperand);
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWRGBInput1(IFXenum eInput)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_RGB_EXT, eSrc);
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND1_RGB_EXT, eOperand);
...
IFXRESULT CIFXDeviceTexUnitOGL::SetHWRGBInterpolatorSource(IFXenum eInput)
{
	...
	IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE2_RGB_EXT, eSrc);
	m_pOGL->glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_RGB_EXT, eOperand);
		...
		if(IFXSUCCESS(rc))
		{
			...
			if(IFXSUCCESS(rc))
			{
				...
				if(m_bCubeMap != bCubeMap)
				{
					m_bCubeMap = bCubeMap;
					if(m_bCubeMap)
						m_eglTarget = GL_TEXTURE_CUBE_MAP_ARB;
					else
...
GLenum CIFXDeviceTexUnitOGL::GetGLInput(IFXenum eInput, GLenum& eOperand)
{
	...
	case IFX_TEXTURE:
		...
		break;
	case IFX_INCOMING:
		eOut = GL_PREVIOUS_EXT;
		break;
	case IFX_DIFFUSE:
		eOut = GL_PRIMARY_COLOR_EXT;
		break;
	case IFX_CONSTANT:
		eOut = GL_CONSTANT_EXT;
		break;
	}
