...
IFXRESULT CIFXRenderDeviceOGL::Initialize(U32 uDeviceNum)
{
	...
	if( IFXSUCCESS( rc ) )
	{
		...
		m_pOGL->glLightModeli(GL_LIGHT_MODEL_COLOR_CONTROL_EXT, GL_SEPARATE_SPECULAR_COLOR_EXT);

		if(m_pOGL->MultiTexSupport())
		{
			GLint glNumTexUnits;
			
			m_pOGL->glGetIntegerv(GL_MAX_TEXTURE_UNITS_ARB, &glNumTexUnits);
    ...
    if( IFXSUCCESS( rc ) )
    {
	    ...
	    if(vaAttribs.m_uData.m_b32BitIndices)
	    {
		    if(uMeshType==0) {
			    ...
			    pvData = IFXAllocate(sizeof(unsigned int)*uNumIndices);
			    for (unsigned int ind = 0; ind< uNumIndices; ind++) {
				    ((unsigned int*)pvData)[ind] = ind;
		    }
		    }
	    ...
	    else
	    {
		    if(uMeshType==0) {
			    ...
			    pvData = IFXAllocate(sizeof(unsigned short)*uNumIndices);
			    for (unsigned short ind = 0; ind< uNumIndices; ind++) {
				    ((unsigned short*)pvData)[ind] = ind;
			    }
		    }
...
void CIFXRenderDeviceOGL::SetActiveTexUnit(U32 uTexUnit)
{
	if(m_uActiveTexUnit != uTexUnit)
	{
		...
		{
			IFXASSERTBOX(m_pOGL->m_bValidOpenGL, "(Error): Making OpenGL call on Invalid context!");
			m_pOGL->glActiveTextureARB(GL_TEXTURE0_ARB + uTexUnit);
			m_pOGL->glClientActiveTextureARB(GL_TEXTURE0_ARB + uTexUnit);
