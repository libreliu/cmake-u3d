...
#define UNDEFINED_INDEX ULONG_MAX
...
private:
	...
	int recordVerticesInFaceUpdates(SmallPtrSet* updated, IFXAuthorFaceUpdate *faceUpdates, 
									Vertex* keep, Vertex* remove);
	int recordTexCoordsInFaceUpdates(FacePtrSet* deleted, SmallPtrSet* updated, 
									IFXAuthorFaceUpdate *faceUpdates,
									Vertex* keep, Vertex* remove);
	int recordAttributesInFaceUpdates(FacePtrSet* deleted, SmallPtrSet* updated, 
	...
	int recordNormalsInFaceUpdates(Vertex* keep, SmallPtrSet* faceSet, 
									IFXAuthorFaceUpdate *faceUpdates);
	int recordNormalUpdate (unsigned long faceIndex, unsigned long keepVertex, 
									unsigned long normalIndex, IFXAuthorFaceUpdate *faceUpdate);
	inline BOOL computeFaceNormal (IFXAuthorFace *face, IFXVector3 *normal);
	inline BOOL texCoordsAreSame (int texCoord1, int texCoord2);
	...
	int recordNormalsInFaceUpdates_NormalNeighborhood(Vertex* keep,  SmallPtrSet* updatedFaces, 
									IFXAuthorFaceUpdate *faceUpdates, BOOL smallNormalChange);

	inline BOOL adjacentFaceExams(FaceExam *fe1, FaceExam *fe2);
	void sortFaceExamList (int numFaces);
	inline float creaseAngle (FaceExam *fe1, FaceExam *fe2);
	int recordNormalsInFaceUpdates_CreaseAngle(Vertex* keep, SmallPtrSet* faceSet, float tolerance, 
									IFXAuthorFaceUpdate *faceUpdates);
...
inline BOOL ContractionRecorder::texCoordsAreSame (int texCoord1, int texCoord2)
{
	IFXVector4 tx1, tx2;
