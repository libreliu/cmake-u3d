...
int ContractionRecorder::recordVerticesInFaceUpdates(SmallPtrSet* updated, IFXAuthorFaceUpdate* faceUpdates,
...
int ContractionRecorder::recordAttributesInFaceUpdates(FacePtrSet* deleted, SmallPtrSet* updated,
													   ...
													   Vertex *keep, Vertex *remove)
{
	...
	while(pUpdatedFace)
	{
		...
		if( pMat->m_uNumTextureLayers  || pMat->m_uDiffuseColors || pMat->m_uSpecularColors )
		{
			...
			while(pDeletedFace)
			{
				int delFaceIndex = pDeletedFace->index;
...
int ContractionRecorder::recordNormalUpdate (unsigned long faceIndex,
											 unsigned long keepVertex,
											 ...
											 IFXAuthorFaceUpdate *faceUpdate)
{
	...
	unsigned long decrNormal = normalIndex;
	unsigned long incrNormal;
	U32 corner = 0;
...
void ContractionRecorder::sortFaceExamList (int numFaces)
{
	int lastFe = 0;
	int allFacesMarked = FALSE;
...
int ContractionRecorder::recordNormalsInFaceUpdates_NormalNeighborhood(Vertex *keep,
...
int ContractionRecorder::recordNormalsInFaceUpdates_CreaseAngle(Vertex *keep,
																SmallPtrSet *faceSet,
																float tolerance,
																IFXAuthorFaceUpdate *faceUpdates)
{
	...
	for (fe = 0; fe < numFaces; fe++)
	{
		...
		unsigned long index = pFace->index;
	...
	int numNormals = 0;
	int numFaceUpdates = 0;

	for (fe = firstFace; fe < firstFace + numFaces; fe++)
	{
		int index = fe % numFaces; // wrap around
		...
		if (faceExam->faceIndex != UNDEFINED_INDEX)
		{
			...
			if ((faceExam->marked) && (fe != firstFace)) // end of a crease angle group
			{
				...
				int start = fe - numNormals;
				int i;
				for ( i = start; i < fe; i++)
				{
					int index = i % numFaces;
...
void ContractionRecorder::generateFaceMap()
{
	...
	int maxFaceIndex = numFaceRemovals - 1;
