							...
							const IFXString& rMixerName,IFXMotionMixer *pMotionMixer)
{
	...
	for(m=0;m<paletteSize;m++)
	{
		...
		IFXTRACE_GENERIC(L"Motion %d: 0x%x \"%ls\"\n",m,(U32)pResource,name.Raw());
	...
	for(m=0;m<paletteSize;m++)
	{
		...
		IFXTRACE_GENERIC(L"Mixer %d: 0x%x \"%ls\"\n",m,(U32)pMixCon,name.Raw());
	}
#endif
		...
		IFXMixerConstruct *pMixCon,const IFXString& rBoneName)
{
	...
	if(motionResource || !pConList->GetNumberElements())
	{
		if(pConList->GetNumberElements())
			IFXTRACE_GENERIC(L"Note: construct 0x%x has resource AND %d submaps\n",
									pMixCon,pConList->GetNumberElements());
		if(!motionResource)
			IFXTRACE_GENERIC(L"Note: construct 0x%x has NULL resource AND no submaps\n",
									pMixCon);
		...
		IFXTRACE_GENERIC(L"find name=\"%ls\" resource=0x%x result==x%x id=%d\n",
				motionResource->GetMotionRef()->GetNameConst().Raw(),
				(U32)motionResource,result,motionId);
							...
							(pConMap=pConList->PostIncrement(context)) != NULL)
	{
		...
		IFXTRACE_GENERIC(L"Construct 0x%x \"%ls\"\n",pOtherCon,othername.Raw());
