...
void IFXMixerQueueImpl::TransitionHead(bool push)
{
	...
	if(IFXSUCCESS(iResult))
	{
		...
		//    IFXTRACE_GENERIC(L"0x%x %.6G change %ls\n",this,GetWorldTime(),change.Out().Raw());
...
void IFXMixerQueueImpl::VerifyQueue(void)
{
	...
	for(m=0;m<length;m++)
	{
		...
		//    IFXTRACE_GENERIC(L"SkelMixer[%d]=0x%x\n",m,pMixer);
		...
		if(pMixer)
		{
			...
			IFXTRACE_GENERIC(L"Warning: SkelMixer[%d]=NULL on SkelParam 0x%x",
			m,(U32)this);
	}
#endif
}
		...
		while( GetNumberHistory() )
		{
			...
			if( fabs(localTime)>IFXMQ_HISTORY_LIFE )
			{
				if(pSkelMixer->EventState()==
					...
					pSkelMixer->Started())
				{
					IFXTRACE_GENERIC(L"Warning: mixer 0x%x \"%ls\" did not end cleanly"
						L" -> event forced\n",pSkelMixer,
	...
	for(m=0;m<2 && m<I32(GetNumberQueued());m++)
	{
...
#if IFXMQ_QUEUEDEBUG
			IFXTRACE_GENERIC(L"On Mixer 0x%x->0x%x",pSkelMixer,
				pSkelMixer);
		...
		if((localTime-max)>IFXMQ_LOOP_THRESHOLD && loop)
		{
			...
			if(!m_noloop)
			{
				//* looping forwards
#if IFXMQ_LOOPDEBUG
				//        IFXTRACE_GENERIC(L"0x%x Loop Forward %.6G>%.6G world=%.6G mixer=%.6G\n",
		...
		else if( (min-localTime) > IFXMQ_LOOP_THRESHOLD && loop )
		{
			if( !m_noloop )
			{
				//* looping backwards
#if IFXMQ_LOOPDEBUG
				IFXTRACE_GENERIC(L"0x%x Loop Backward %.6G<%.6G world=%.6G mixer=%.6G\n",
	...
	for(m=0; m < (I32)((history=GetNumberHistory())+GetNumberQueued());m++)
	{
			...
			pSkelMixer!=pBlendMixer))
		{
			...
			if(pSkelMixer->EventState()==IFXMotionMixer::IFXSX_WaitEnd)
			{
				...
				{
#if IFXMQ_QUEUEDEBUG
					IFXTRACE_GENERIC(L"On Mixer 0x%x->0x%x\n",pSkelMixer,
						pSkelMixer);
#endif
					pSkelMixer->Started()=false;
				}
...
#if IFXMQ_LOOPDEBUG
		IFXTRACE_GENERIC(L"0x%x AdjustForGap index=%d\n",this,index);
