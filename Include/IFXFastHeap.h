...
public:
	...
	*/
	...
	/**
		...
		@param A U32 representing the heap context.
...
private:
	struct Node
	{
		...
		void Set(const K& in_Key, const T& in_Data, U32* in_Ctx)
		{
			Key = in_Key;
			Data = in_Data;
			pContext = in_Ctx;
	...
	U32* NewContext(U32);
	void FreeContext(U32* in_Ctx);

	Node* m_pHeap;
	U32* m_pHeapContext;
	U32 m_NextFreeContextId;
	U32* m_pNextFreeContext;
	U32 m_Size;
	U32 m_MaxSize;
};
...
inline IFXFastHeap<K, T>::IFXFastHeap(U32 in_Size)
{
	...
	m_pHeapContext = new U32[in_Size];
...
template<class K, class T>
inline U32 IFXFastHeap<K, T>::Insert(const K& in_Key, const T& in_Data)
{
	m_pHeap[m_Size].Set(in_Key, in_Data, NewContext(m_Size));
	U32 Ctx = (U32) m_pHeap[m_Size].pContext;
	HeapInsert(m_Size);
	++m_Size;
	return Ctx;
}
...
template<class K, class T>
void IFXFastHeap<K, T>::Remove(U32 in_Context)
{
	IFXASSERT(in_Context >= ((U32)m_pHeapContext) 
			&& in_Context < (U32)(m_pHeapContext + m_MaxSize) );
...
template<class K, class T>
void IFXFastHeap<K, T>::Reinsert(U32 in_Context, const K& in_NewKey)
{
	IFXASSERT(in_Context >= ((U32)m_pHeapContext) 
			&& in_Context < (U32)(m_pHeapContext + m_MaxSize) );
	U32 NodeIdx = *(U32*)in_Context;
...
template<class K, class T>
U32* IFXFastHeap<K, T>::NewContext(U32 in_Idx)
{
	if(m_pNextFreeContext)
	{
		U32* pContext = NULL;
		pContext = m_pNextFreeContext;
		m_pNextFreeContext = (U32*)*pContext;
		*pContext = in_Idx;
		return pContext;
	}
...
template<class K, class T>
void IFXFastHeap<K, T>::FreeContext(U32* in_Ctx)
{
	*in_Ctx = (U32)m_pNextFreeContext;
